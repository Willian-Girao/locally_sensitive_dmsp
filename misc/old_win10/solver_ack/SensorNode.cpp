...
//Constructor.
SensorNode::SensorNode(string instanceFileName, int sensorId, bool shouldDebug, int shouldDebugLevel, bool awaitAck, double radius) {
	//Starting debug properties
	debug = shouldDebug;
	debugLevel = shouldDebugLevel;
	mpiTimeDebuging = MPI_Wtime();
	noAckAwait = awaitAck;
	noAckFlagHelperEnumerNodes = true; //Helper flag
	...
	alphaValue = radius; //Radius of communication range
...
SensorNode::~SensorNode() {
	...
	if (nodeId == 0) {
		...
		cout << "Ack await:      " << noAckAwait << endl;
		switch (selected)
		{
			case GREEDY:
			{
...
void SensorNode::debugMesseging(int receiver, string msg, int payload) {
	if (debug)
	{
		...
		case 2:
			...
			break;
		case 3:
			if (msg == "MSG_REQUEST" || msg == "MSG_ENUMERNODES" || msg == "MSG_SERVED")
			{
				cout << "\n" << nodeId << " " << msg << " " << receiver << "	- " << (tf - mpiTimeDebuging) << "ms" << endl;
			}
...
bool SensorNode::checkAllMsgEnumernodesReceived(void) {
	...
	//No need to await for ack msgs
	if (noAckAwait)
	{
		int countAux = 0;
		int accountedNeighbots = 0;

		//Must check if the node that did reply didn't actually replied with 0 - test graph_0 sensor 1 (no neighbots) case
		for (int i = 0; i < my_neighbors_ids.size(); i++)
		{
			if (backtracking)
			{
				if (neighbors_MSG_ENUMERNODES_buffer[my_neighbors_ids[i]].second >= 0)
				{
					countAux += neighbors_MSG_ENUMERNODES_buffer[my_neighbors_ids[i]].second;
				}
				accountedNeighbots++;
			}
			else {
		...
		if (countAux > 0)
		{
			noAckFlagHelperEnumerNodes = false;
			return true;
		}
		else if (countAux == 0 && accountedNeighbots > 1) {
			if (backtracking)
			{
				noAckFlagHelperEnumerNodes = false;
				return true;
...
void SensorNode::muleOn1stSensorStart(void) {
	noAckFlagHelperEnumerNodes = true; //Helper
...
void SensorNode::msgServedReceived() {
	...
	if (!served)
	{
		...
		if (my_neighbors_ids.size() - 1 == 0)
		{
			...
			//Some stuff - ERROR MSG POSSIBLE FIX
			for (int i = 0; i < my_neighbors_ids.size(); i++)
			{
			...
			case GREEDY:
				for (int i = 0; i < my_neighbors_ids.size(); i++)
				{
					...
					if (my_neighbors_ids[i] != parentId)
					{
						...
						//Updating metrics variables
						will += noAckAwait ? 2 : 2; //Increase local count by x2 (account for N(u) reply)
					}
				}
				break;
			case CONVEX_HULL:
...
void SensorNode::msgAckBeingServedReceived(void) {
	...
	if (checkAllNeighborFlagAckBeingServedReceived())
	{
		...
		/*
			Probably error here: will is not being sent when there is no waiting for ack
		*/
...
void SensorNode::msgEnumernodesReceived(void) {
	...
	//Stops the sensor from receiving more msg in the case with no ack
	if (noAckFlagHelperEnumerNodes)
	{
...
void SensorNode::msgSendMuleReceived(void) {
	//Helper
	noAckFlagHelperEnumerNodes = true;
